				...
				for (Canton canton : line.getCantonList()) {
					if (canton.hasIncident() && Integer.parseInt(canton.getId()) != (line.getCantonList().size() - 1)) {
						...
						if (nextCanton.isFree() && !nextCanton.hasIncident() && nextCanton.getStation().getNumReserveTrain() > 0 && ( canton.isFree() || canton.hasTrainBlocked())){
							...
							Train newTrainReserve = new Train(canton, 0, Constants.TRAIN_BASIC_SPEED, Train.RESERVE_TYPE);
							line.addTrain(newTrainReserve);
							newTrainReserve.setCurrentPosition(canton.getStation().getPosition());
							try {
								canton.getStation().enter(newTrainReserve);
							} catch (TerminusException e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
							}
							newTrainReserve.start();
						}
