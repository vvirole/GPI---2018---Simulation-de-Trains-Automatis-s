...
public class LineController extends Observable implements Runnable {
	...
	public void run() {
		...
		while(line.isWorking() && time <= GUIConstants.MAX_DURATION){
			...
			passenger = passenger/line.getNbCanton();
			SimulationData.getInstance().addTravelerData(getTime(), passenger);
			SimulationData.getInstance().addSatisfactionData(getTime(), satisfaction);
